...
def create_and_populate_google_doc(image_file, gpt_output_text, user_email):
    ...
    try:
        ...
        # Add a title or introductory text before the GPT content if desired
        # This part ensures your document has a proper title from session state before GPT's output
        doc_html_prefix = (
            f"<h1>Rightsize AI Analysis for {st.session_state.room} ({st.session_state.goal})</h1>"
            f"<p>Generated by Rightsize AI.</p>" # Optional intro paragraph
        )

        # Convert the GPT's Markdown text to HTML
        # Use extensions like 'fenced_code' for code blocks, 'tables' for tables etc.
        # Ensure you include all necessary extensions based on the Markdown GPT might output.
        gpt_output_html = markdown.markdown(gpt_output_text, extensions=['fenced_code', 'tables', 'nl2br'])

        # Combine the prefix and the GPT-generated HTML
        full_html_content = f"{doc_html_prefix}\n{gpt_output_html}"
        ...
        # You need to wrap html_file_obj in MediaIoBaseUpload here.
        ...
        uploaded_html_file_in_drive = drive_service.files().create(
            body=html_file_metadata, media_body=html_media_body, fields="id" # Use html_media_body here!
        ).execute()
        ...
        converted_doc_metadata = {
            "name": "Rightsize AI Output", # Name the final document
        ...
        document_id = main_doc_from_html.get("id") # Corrected: Use get("documentId") not get("id") for Docs
        ...
        # # Insert the combined text content -- DEPRECATING, THIS IS HOW THE MARKDOWN WAS ORIGINALLY INSERTED
        # doc_content = (
